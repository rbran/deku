...
fn emit_struct(input: &DekuData) -> Result<TokenStream, syn::Error> {
    ...
    tokens.extend(quote! {
        impl #imp ::#crate_::DekuReader<#lifetime, #ctx_types> for #ident #wher {
            fn from_reader_with_ctx<R: ::#crate_::acid_io::Read + ::#crate_::acid_io::Seek>(__deku_reader: &mut ::#crate_::reader::Reader<R>, #ctx_arg) -> core::result::Result<Self, ::#crate_::DekuError> {
                #read_body
            }
        }
    });
    ...
    if input.ctx.is_some() && input.ctx_default.is_some() {
        ...
        tokens.extend(quote! {
            impl #imp ::#crate_::DekuReader<#lifetime> for #ident #wher {
                fn from_reader_with_ctx<R: ::#crate_::acid_io::Read + ::#crate_::acid_io::Seek>(__deku_reader: &mut ::#crate_::reader::Reader<R>, _: ()) -> core::result::Result<Self, ::#crate_::DekuError> {
                    #read_body
                }
            }
        });
...
fn emit_field_read(
    ...
    let seek = if let Some(num) = &f.seek_from_current {
        quote!{
            use ::#crate_::acid_io::Seek;
            // TODO: use ?
            __deku_reader.seek(::#crate_::acid_io::SeekFrom::Current(i64::try_from(#num).unwrap())).unwrap();
        }
    } else {
        quote!{}
    };
...
pub fn emit_from_reader(
    ...
    quote! {
        impl #imp ::#crate_::DekuContainerRead<#lifetime> for #ident #wher {
            #[allow(non_snake_case)]
            fn from_reader<'a, R: ::#crate_::acid_io::Read + ::#crate_::acid_io::Seek>(__deku_input: (&'a mut R, usize)) -> core::result::Result<(usize, Self), ::#crate_::DekuError> {
                #body
            }
        }
    }
